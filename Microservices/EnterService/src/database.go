...
func NewDatabase(dbpath string) (*database, error) {
	...
	sqlStmt := `
	create table if not exists door (id integer not null primary key, door text, status text, timestamp text);
	`
...
func (db database) Insert(msgDoor DoorMQ) error {
	...
	stmt, err := db.conn.Prepare("INSERT INTO door(door, status, timestamp) values(?,?,?)")
	if err != nil {
		return err
	}

	_, err = stmt.Exec(msgDoor.Door, msgDoor.Status, msgDoor.Timestamp)
	if err != nil {
		return err
	}
...
func (db database) GetAll() ([]DoorMQ, error) {
	...
	rows, err := db.conn.Query("SELECT door, status, timestamp FROM door")
	if err != nil {
		return nil, err
	}

	for rows.Next() {
		err = rows.Scan(&door, &status, &timestamp)
		if err != nil {
			return nil, err
		}
		doorsSlice = append(doorsSlice, DoorMQ{Door: door, Status: status, Timestamp: timestamp})
