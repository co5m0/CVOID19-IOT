...
func main() {
	...
	go func() {
		for msg := range msgs {
			...
			fmt.Printf("table: %s\t seat: %s\t status: %s\t timestamp: %s\n", body.Table, body.Seat, body.Status, body.Timestamp)
	...
	go func() {
		...
		for msg := range msgToClient {
            valueInt, err := strconv.ParseFloat(msg.Status, 64)
			if err != nil {
				log.Fatalln("Error converting value from string to int")
			}
			if err := ref.Update(ctx, map[string]interface{}{
				fmt.Sprintf("table_%s/seat_%s", msg.Table, msg.Seat): map[string]interface{}{
                    "value": valueInt,
                    "time": msg.Timestamp,
                },
			}); err != nil {
